...
public class RentController {
	...
	public void save(long cpf) {
		...
		try {
			...
			if (client.getBooksRent() == 3) {
				messengerJFrame.printErrorMessage(window, "Cliente j� possui maximo de livros alugados!");
				return;
			} else {
				...
				if (bookTemp.isAvaliable() == false) {
					messengerJFrame.printErrorMessage(window, "Livro j� est� alugado!");
					return;
				} else {
			...
			messengerJFrame.printErrorMessage(window, "Campo Inv�lido!");
		}
	}
	...
	public void showRent() {
		System.out.println("-----------------------------\n");
		System.out.println(String.format("%-20s", "|C�digo do Aluguel") + "\t"
