	...
	public void remove() {
		...
		if (cli.getBooksRent() > 0) {
			messengerJFrame.printErrorMessage(window,
					"Cliente possui livros alugados, para cancelar cadastro È necessario devolver todos os livros antes");
			return;
		} else {
		...
		try {
			...
			if (realTimeScrenn == REGISTERFORM) {
				if (clientExist(cpfTemp)) {
					...
					messengerJFrame.printErrorMessage(window, "CPF È inv·lido");
				} else {
					if (!nameValidator(nameTemp) || name == null) {
						messengerJFrame.printErrorMessage(window, "Nome È inv·lido");
					} else {
				...
				if (clientExist(cpfTemp)) {
					messengerJFrame.printErrorMessage(window, "RG j· existe.");
				} else if (cpfValidator(cpfTemp) == false) {
					messengerJFrame.printErrorMessage(window, "RG È inv·lido!");
				} else {
					if (!nameValidator(nameTemp) || name == null) {
						messengerJFrame.printErrorMessage(window, "Nome È inv·lido!");
					} else {
						...
						;
						messengerJFrame.printSucessMesenge(window, "Edi√ß√£o realizada com sucesso!");
					}
				}
			}
		} catch (Exception e) {
